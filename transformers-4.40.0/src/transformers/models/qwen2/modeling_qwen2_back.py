...
class Qwen2SdpaAttention(Qwen2GraphAttention):
    """
    Qwen2 graph-aware attention module using torch.nn.functional.scaled_dot_product_attention.
    This module inherits from `Qwen2GraphAttention` and adapts the forward pass to use SDPA API
    while preserving graph relationship integration.
    """
    ...
    def forward(
        ...
        output_attentions: bool = False,
        use_cache: bool = False,
        graph_sprels: Optional[torch.Tensor] = None,  # 图关系参数
        **kwargs,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        # 如果需要输出注意力权重，回退到手动实现的父类方法
        if output_attentions:
            logger.warning_once(
                "Qwen2Model is using Qwen2GraphSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` "
        ...
        # 如果有图关系但没有使用自定义的图处理，也回退到手动实现
        if graph_sprels is not None and self.sprel_linear is None:
            return super().forward(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
        ...
        if attention_mask is not None:
            ...
            attention_mask = torch.zeros(bsz, 1, q_len, kv_seq_len, device=query_states.device,dtype=query_states.dtype)
            # 如果需要因果掩码
        ...
        # 处理图关系 (无论 attention_mask 是否为 None)
        # print(f"\n\n\ngraph_sprels: {graph_sprels}\nself.sprel_linear: {self.sprel_linear}\n\n")
        if graph_sprels is not None and self.sprel_linear is not None:# [batch_size, n, n]
        ...
        # SDPA with memory-efficient backend bugfix
...
class Qwen2SdpaAttention_back(Qwen2Attention): 
    """
...
class Qwen2DecoderLayer_back(nn.Module):
...
class Qwen2DecoderLayer(nn.Module): #add_graph
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        graph_sprels: Optional[torch.Tensor] = None #add_graph
