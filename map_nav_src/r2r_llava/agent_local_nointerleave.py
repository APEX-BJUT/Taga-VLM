...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            #infer llava-nav
            llava_a_t=self.infer_llava_nav(obs,traj,nav_inputs)
            ...
            if self.feedback == 'teacher' or self.feedback == 'sample': # in training
                ...
                # a_t_stop = a_t == 0
                a_t_stop = [llava_a_t_1 == 0 for llava_a_t_1 in llava_a_t]
                # a_t_stop = llava_a_t == 0
            ...
            for i in range(batch_size):
                if a_t_stop[i] or ended[i] or nav_inputs['no_vp_left'][i] or (t == self.args.max_action_len - 1):
                    ...
                    cpu_a_t.append(nav_vpids[i][llava_a_t[i]])   
        ...
        return traj
    def get_connection(self,graph,gmap_vpids,traj_vpids,curr_vpid):
        connected_text = " Connected with : "
        connected_idx = []
        if curr_vpid not in traj_vpids:#candidates
            for idx , vpid in  enumerate(gmap_vpids):
                if vpid in traj_vpids:
                    if graph.has_edge(curr_vpid,vpid):
        ...
        connected_text += "<idx>{}</idx>".format(connected_idx)
        return connected_text
    def infer_llava_nav(self,obs,trajs,nav_inputs):
        a_t=[]
        for idx , ob in enumerate(obs):
            his_curr_node_image_file = []
            cand_node_image_file=[]
            traj = []
            for k, vpid in enumerate(nav_inputs['gmap_vpids'][idx]):
                if nav_inputs['gmap_visited_masks'][idx][k] == True:
                    traj.append(vpid)
            #local del history_cand
            local_gmap_vpids = nav_inputs['gmap_vpids'][idx].copy()
            for vpid in nav_inputs['gmap_vpids'][idx]:
                if vpid not in nav_inputs['vp_cand_vpids'][idx] and vpid not in traj:
                    # local_gmap_vpids.remove(vpid)
                    idx_to_remove = nav_inputs['gmap_vpids'][idx].index(vpid)
            # 如果有节点要删除，更新gmap_vpids和gmap_pair_dists
            if indices_to_remove:
                # 排序索引（从大到小），这样删除时不会影响后续索引
                indices_to_remove.sort(reverse=True)
                
                # 删除gmap_vpids中的节点
                for idx in indices_to_remove:
                    del nav_inputs['gmap_vpids'][idx]
                
                # 更新gmap_pair_dists - 删除对应的行和列
                keep_indices = [i for i in range(len(nav_inputs['gmap_pair_dists'][idx])) if i not in indices_to_remove]
                nav_inputs['gmap_pair_dists'][idx] = np.array(nav_inputs['gmap_pair_dists'][idx])[np.ix_(keep_indices, keep_indices)]
            ...
            # get history and current image
            question = "<nav>"+ob['instruction']+"</nav>"+"historical observation:{"
            for i,  vpid in enumerate(traj):
                if vpid != traj[-1]:
                    # connection_text = self.get_connection(self.graphs[ob["scan"]],local_gmap_vpids,traj,vpid)
                    ...
                    question += "<node><idx>{}</idx></node>".format(i+1)  #v4

                else:
                    # connection_text = self.get_connection(self.graphs[ob["scan"]],local_gmap_vpids,traj,vpid)
                    his_curr_node_image_file.append(ob["scan"] +'/matterport_skybox_images/' + vpid + '_skybox_small_cropped.jpg')
                    # question += "};current observation:{" + "<node><image></node>" + "};candidate observation:{"                     #v5
                    question += "};current observation:{<node><idx>"+str(i+1)+"</idx></node>};candidate observation:{"  #v4

            for j ,vpid_cand in enumerate(local_gmap_vpids):
                if j !=0:
                    if nav_inputs['gmap_visited_masks'][idx][j] == False:
                        # connection_text = self.get_connection(self.graphs[ob["scan"]],local_gmap_vpids,traj,vpid_cand)
                        cand_node_image_file.append(vpid_cand)
                        # question += "<node><idx><"+str(j)+"></idx><image></node>" #v5
                        question += "<node><idx>{}</idx></node>".format(j) #v4
            question += "}"
            gmap_pair_dists = nav_inputs['gmap_pair_dists'][idx]
            view_imgs = {}
            for vp in traj:
                imgs = self.views_file['%s_%s'%(ob["scan"], vp)]
                # cand views
                nav_cands = self.scanvp_cands['%s_%s'%(ob["scan"], vp)]
                for k, v in nav_cands.items():
                    try:
                        view_imgs[k].append(imgs[v])
                    except:
                        view_imgs[k] = [imgs[v]]
                # for k, v in nav_cands.items():
                    
                #     if k not in view_imgs.keys():
                #         view_imgs[k] = [imgs[v]]
                #     else:
                #         view_imgs[k].append(imgs[v])
            if len(traj) > 1:
                for visted_vp in traj:
                    if visted_vp in view_imgs.keys():
                        del view_imgs[visted_vp]
            ...
            for image_file in his_curr_node_image_file:
                ...
                image=Image.open("/root/liujiaxing/LLaVA-NeXT-graph/data/mp3d_data/"+image_file).convert("RGB")

                
                images.append(image)
            # for id, cand in view_imgs.items():
            #     if len(cand)>1:
            #         cand_image  = cv2.hconcat(cand)
            #     else:
            #         cand_image = cand[0]
            #     cand_image = Image.fromarray(cand_image).convert("RGB")
            #     images.append(cand_image)
            #     cand_id.append(id)
            for cand in cand_node_image_file:
                cand_img_list = view_imgs[cand]
                if len(cand_img_list)>1:
                    cand_image  = cv2.hconcat(cand_img_list)
                else:
                    cand_image = cand_img_list[0]
                cand_image = Image.fromarray(cand_image).convert("RGB")
                images.append(cand_image)
                cand_id.append(cand)
            #infer
            image_tensors = process_images(images, self.llava_image_processor, self.llava_model.config)
            ...
            input_ids = tokenizer_image_token(prompt_question, self.llava_tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).to(self.device)
            num_image_token = (input_ids == IMAGE_TOKEN_INDEX).sum()
            num_image = len(image_tensors)
            ...
            self.llava_model.graph_sprels = nav_inputs['gmap_pair_dists']

            if num_image_token==num_image_token:
                image_sizes = [image.size for image in images]
                cont = self.llava_model.generate(
                    input_ids,
                ...
                text_outputs = self.llava_tokenizer.batch_decode(cont, skip_special_tokens=False)[0]
                # node_number = None
                if "stop" in text_outputs.lower():
                    a_t.append(0)
                elif "node" in  text_outputs.lower():
                    node_number = re.findall(r'\b\d+\b', text_outputs)
                    if len(node_number) >= 1:
                        # print(len(node_number[0]))
                        if len(node_number[0])<3:
                            if int(node_number[0]) < len(local_gmap_vpids):
                            #先检查是否是当前点
                            # if nav_inputs['gmap_visited_masks'][idx][int(node_number[0])] == True and nav_inputs['gmap_visited_masks'][idx][int(node_number[0])+1] == False:
                                if local_gmap_vpids[int(node_number[0])] == ob['viewpoint']:    
                                    a_t.append(0)
                                    print("can't choose cuurent node!!!!!")
                                else:
                                    a_t_local_vpid = local_gmap_vpids[int(node_number[0])]
                                    
                                    a_t.append(nav_inputs['gmap_vpids'][idx].index(a_t_local_vpid))
                            else:
                                a_t.append(0)
                                print("node idx out of range!!!!!")
                        else:
                            a_t.append(0)
                            print("num str too lange!!!!!")
                    else:#没有数字，格式出错
                        a_t.append(0)
                        print("format error!!!!!")
                else:#没有标签，格式出错
