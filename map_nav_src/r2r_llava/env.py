...
import numpy as np
from r2r.eval_utils import cal_cls, cal_dtw
from utils.data import angle_feature, get_all_point_angle_feature, load_nav_graphs, new_simulator
...
class R2RNavBatch(object):
    ...
    def __init__(
        ...
        self.env = EnvBatch(connectivity_dir, feat_db=view_db, batch_size=batch_size, scan_data_dir=scan_data_dir)
    ...
    def _get_gt_trajs(self, data):
        gt_trajs = {x["instr_id"]: (x["scan"], x["path"]) for x in data if len(x["path"]) > 1}
        return gt_trajs
