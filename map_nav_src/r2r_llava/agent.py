...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        return {
            'view_img_fts': None, 'loc_fts': None, 
            'nav_types': batch_nav_types, 'view_lens': None, 
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        return {
            'gmap_vpids': batch_gmap_vpids, 'gmap_img_embeds': None, 
        ...
        for t in range(self.args.max_action_len):
            ...
            nav_inputs.update(
                self._nav_vp_variable(
                    obs, gmaps, None, pano_inputs['cand_vpids'], 
    ...
    def infer_llava_nav(self,obs,trajs,nav_inputs):
        ...
        for idx , ob in enumerate(obs):
            ...
            if dataset == 'mp3d':
                ...
                # for vp in traj:
                #     # imgs = self.views_file['%s_%s'%(ob["scan"], vp)]
                #     images_path = os.path.join(self.mp3d_views_file,ob["scan"],vp)
                #     # cand views
                #     nav_cands = self.scanvp_cands['%s_%s'%(ob["scan"], vp)]
                #     for k, v in nav_cands.items():
                        
                #         try:
                #             pil_img = image=Image.open(os.path.join(images_path,str(v))+'.jpg').convert("RGB")
                #             img = np.array(pil_img)
                #             view_imgs[k].append(img)
                #         except:
                #             pil_img = image=Image.open(os.path.join(images_path,str(v))+'.jpg').convert("RGB")
                #             img = np.array(pil_img)
                #             view_imgs[k] = [img]
                ...
                for vp in traj:
                    # imgs = self.views_file['%s_%s'%(ob["scan"], vp)]
                    images_path = os.path.join(self.hm3d_views_file,ob["scan"],vp)
                    # cand views
                    nav_cands = self.scanvp_cands['%s_%s'%(ob["scan"], vp)]
                    for k, v in nav_cands.items():
                ...
                if "stop" in text_outputs.lower():
                    ...
                    if len(node_number) >= 1:
                        ...
                        if len(node_number[0])<3:
                            if int(node_number[0]) < len(nav_inputs['gmap_vpids'][idx]):
                            #先检查是否是当前点
                            # if nav_inputs['gmap_visited_masks'][idx][int(node_number[0])] == True and nav_inputs['gmap_visited_masks'][idx][int(node_number[0])+1] == False:
                                if nav_inputs['gmap_vpids'][idx][int(node_number[0])] == ob['viewpoint']:    
                                    a_t.append(0)
                                    print("can't choose cuurent node!!!!!")
                                else:
                                    a_t.append(int(node_number[0]))
                            else:
                                a_t.append(0)
                    ...
                    a_t.append(0)#random.choice(action_space)
                    print("format error!!!!!")
                # print(text_outputs)
