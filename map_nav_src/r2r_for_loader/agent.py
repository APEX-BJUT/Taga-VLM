...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            llava_a_t=self.get_llava_input_output(obs,traj,nav_inputs,nav_targets)
        ...
        #     # 处理完毕后关闭此次连接
        #     sock.close()
        #     print("连接已关闭，等待下一个客户端连接...")


        return traj
    def get_llava_input_output(self,obs,trajs,nav_inputs,nav_targets):
        a_t=[]
        for idx , ob in enumerate(obs):
            ...
            for i,  vpid in enumerate(traj):
                if vpid != traj[-1]:
                    ...
                    question += "<node><idx>"+str(i+1)+"</idx><image></node>"
                else:
                    his_curr_node_image_file.append(ob["scan"] +'/matterport_skybox_images/' + vpid + '_skybox_small.jpg')
                    question += "};current observation:{" + "<node><idx>"+str(i+1)+"</idx><image></node>" + "};candidate observation:{"
            for j ,vpid_cand in enumerate(nav_inputs['gmap_vpids'][idx]):
                if j !=0:
                    if nav_inputs['gmap_visited_masks'][idx][j] == False:
                        cand_node_image_file.append(ob["scan"]+'_'+vpid_cand)
                        question += "<node><idx>"+str(j)+"</idx><image></node>"
            question += "}"
            ...
            for vp in traj:
                ...
                for k, v in nav_cands.items():
                    ...
                    #     view_imgs[k].append(v)
                    # except:
                    #     # view_imgs[k] = [imgs[v]]
                    #     view_imgs[k] = [v]
                    try:
                        view_imgs[k].append({vp:v})
                    except:
                        view_imgs[k] = [{vp:v}]
            ...
            message = {
                "his_curr_node_images" :his_curr_node_image_file,
                "cand_node_image" :cand_node_image_file,
                "cand_view_imgs_idx" :view_imgs,
                "question":question,
                "lable":nav_targets.cpu().numpy().tolist() ,
                "gmap_pair_dists":gmap_pair_dists.cpu().numpy().tolist(),
                "id":ob['instr_id']
            }

            if self.begin_training == True:
                request_data = self.socket_server_send_input(message)
            if self.begin_training == False:
                while True:
                    request_data = self.socket_server_send_input(message)
                    # print("request_data:"+request_data)
                    if request_data == "begin_training":
                        self.begin_training=True
                        print("开始训练")
                        break
            
            # if self.get_item_test_count >=3:
            #     text_outputs = self.socket_server_get_at()
            # else:
            #     text_outputs = 'none'
            ...
            if "stop" in text_outputs.lower():
                ...
                if len(node_number) >= 1:
                    if int(node_number[0]) < len(nav_inputs['gmap_visited_masks'][idx]):
                        ...
                        if nav_inputs['gmap_vpids'][idx][int(node_number[0])] == ob['viewpoint']:
                            a_t.append(0)
                            print("can't choose current node!!!!!")
                        else:
    ...
    def socket_server_send_input(self, message):
        sock, addr = self.server.accept()
        # print(f"客户端 {addr} 已连接")
        self.get_item_test_count+=1
        try:
            # 接收客户端数据
            data = sock.recv(1024).decode('utf-8')
            # print(f"收到客户端消息: {data}")
            ...
            # 处理完毕后关闭此次连接
            sock.close()
            # print("输入信息连接已关闭，等待输出数据...")
        return data
    
    def socket_server_get_at(self):
        sock, addr = self.server.accept()
        # print(f"客户端 {addr} 已连接")
        try:
            # 接收客户端数据
