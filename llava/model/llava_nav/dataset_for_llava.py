...
class ReverieTextPathData(object):
    def __init__(
        self, anno_files, img_ft_file, dep_ft_file, obj_ft_file, scanvp_cands_file, connectivity_dir, img_file,
...
class R2RTextPathData(ReverieTextPathData):
    def __init__(
        self, anno_files, img_ft_file, dep_ft_file, scanvp_cands_file, connectivity_dir, img_file,
        image_feat_size=2048, image_prob_size=1000, depth_feat_size=128, angle_feat_size=4,
        max_txt_len=100, in_memory=True, act_visited_node=False,
        val_sample_num=None
    ):
        super().__init__(
            anno_files, img_ft_file, dep_ft_file, None, scanvp_cands_file, connectivity_dir, img_file,
    ...
    def get_scanvp_image(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)
        if self.in_memory and key in self._img_store:
            view_imgs = self._img_store[key]
        else:
            with h5py.File(self.img_file, 'r') as f:
                view_imgs = f[key][...].astype(np.float16)
            if self.in_memory:
                self._img_store[key] = view_imgs
        return view_imgs
    ...
    def get_input_for_sap_dataset(
        ...
        traj_view_img_fts, traj_view_dep_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
            last_vp_angles, cand_viewids= self.get_traj_pano_fts(scan, gt_path)
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        Each token consists of (img_fts, loc_fts (ang_fts, box_fts), nav_types)
        '''
        traj_view_img_fts, traj_view_dep_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids ,traj_view_imgs ,view_imgs = [], [], [], [], [], [] ,{}

        for vp in path:
            # view_fts, dep_fts = self.get_scanvp_feature(scan, vp)
            # imgs = self.get_scanvp_image(scan, vp)
            ...
            for k, v in nav_cands.items():
                ...
                # view_imgs.append({"cand_id":k,"view_id":v[0]})
                try:
                    view_imgs[k].append({vp:v[0]})
                except:
                    view_imgs[k] = [{vp:v[0]}]
                # view_imgs.append(imgs[v[0]])
                # TODO: whether using correct heading at each step
                view_angle = self.all_point_rel_angles[12][v[0]]
                view_angles.append([view_angle[0] + v[2], view_angle[1] + v[3]])
                cand_vpids.append(k)
            # non cand views
            # view_img_fts.extend([view_fts[idx] for idx in range(36) if idx not in used_viewidxs])
            # view_imgs.extend([imgs[idx] for idx in range(36) if idx not in used_viewidxs])
            ...
            # view_imgs = np.stack(view_imgs, 0)
            ...
            # traj_view_imgs.append(view_imgs)
        ...
        return traj_view_img_fts, traj_view_dep_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, last_vp_angles ,view_imgs