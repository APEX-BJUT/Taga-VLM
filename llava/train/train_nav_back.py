...
#导航
from llava.model.llava_nav.dataset_for_llava import R2RTextPathData
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item_nav(self, i) -> Dict[str, torch.Tensor]:
        ...
        end_vp_pos_ratio=0.2
        r = np.random.rand()
        if r < end_vp_pos_ratio:
            end_vp_type = 'pos'
        elif r < 0.6:
            end_vp_type = 'neg_in_gt_path'
        else:
            end_vp_type = 'neg_others'
        inputs = self.nav_db.get_input(i, end_vp_type, return_act_label=True)

        output = {}

        # output['txt_ids'] = torch.LongTensor(inputs['instr_encoding'])
        output['instruction'] = inputs['instruction']
        ...
        # output['traj_view_imgs'] = inputs['traj_view_imgs']
        output['scan'] = inputs['scan']
        
        local_cand_vpids = output['traj_cand_vpids'][-1]
        current_vpid = output['traj_vpids'][-1]
        history_vpids = output['traj_vpids'][:-1]
        ...
        for vpid in history_vpids:
            input_vpids.append(vpid)
            input_conversations[0]['value']+='<image>'

        input_vpids.append(current_vpid)
        input_conversations[0]['value']+="\nImagine you are a robot programmed for navigation tasks. You have been given images of historical observations and a image of current observation <image>.Your assigned task is: "
        input_conversations[0]['value']+=output['instruction']
        input_conversations[0]['value']+= "Please analyze this series of images and determine your next action based on the task, selecting the corresponding view of the next action's destination from the following candidate views. The candidate views are: "

        for idx, vpid in enumerate(local_cand_vpids):
            input_vpids.append(vpid)
            input_conversations[0]['value']+="\nView "+str(idx+1)+":<image>"

        input_conversations[1]['value'] = "The answer is View "+str(output['local_act_labels'])+"."

        input_images= []
        for vpid in input_vpids:
            image_file = output['scan'] + '_' + vpid + '.jpg'
        ...
        has_image = ("image" in input_data) or ("video" in input_data)
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image)
        ...
        data_dict["id"] = input_data.get("id", i)
        return data_dict
